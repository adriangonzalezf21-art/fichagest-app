...
export default function AdminUsersPage() {
  ...
  const toggleActive = async (u: ProfileRow, nextActive: boolean) => {
    ...
    if (u.company_id && u.company_id !== myCompanyId) {
      alert("No puedes modificar usuarios de otra empresa.");
      return;
    }
  ...
  const setRole = async (u: ProfileRow, nextRole: "admin" | "worker") => {
    if (!canManageAdminRoles({ is_owner: iAmOwner })) {
      alert("Solo el OWNER de plataforma puede asignar o quitar permisos de admin.");
      return;
    }
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : "Rol no permitido.");
      return;
    }
  ...
  const deleteUserProfile = async (u: ProfileRow) => {
    ...
    if (u.company_id && u.company_id !== myCompanyId) {
      alert("No puedes eliminar usuarios de otra empresa.");
      return;
    }
